...
*/
public class SwitchUserOnlineTimeChart{
	private static SavibindingtableDao bingTableDao = new SavibindingtableDao();
	private Long switchId;
	
	public SwitchUserOnlineTimeChart(Long switchId) {
...
private static XYDataset createDataset(Long switchId) {
    ...
    List staticlist = bingTableDao.getSwitchhisOnlineStaticUsers(switchId);
    List slaaclist = bingTableDao.getSwitchhisOnlineSlaacUsers(switchId);
    List dhcplist = bingTableDao.getSwitchhisOnlineDhcpUsers(switchId);
...
public void saveChartPicture(Long switchId,JFreeChart chart){
	...
	try { 
		fos_jpg=new FileOutputStream(path + "用户在线时间 Switch_" + switchId + ".jpg"); 
	...
	@SuppressWarnings({"unchecked"})
    private static int searchOnTimeList(List list,long time){
