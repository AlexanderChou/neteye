...
*/
public class MaxFilteringNumMeterChart1 {
	private static IfInterfaceDao interfaceDao = new IfInterfaceDao();
	private Long switchId;
	
	public MaxFilteringNumMeterChart1(Long switchId) {
	...
	public static void generateMaxFilteringNumPic(String val) {
		try {
			...
			FileOutputStream fos = new FileOutputStream(path+"Max Filtering-Num.png");
			...
			g.setFont(f1);
			g.drawString("Max Filtering-Num", x - 27 - "Max Filtering-Num".length(), y + 42);
			g.setFont(f);
			int k = 0;
			Double range = value;
			int mag = 1; //数量级
