...
import com.totalIP.dao.LossDao;
import com.totalIP.dao.LossDaov6;
import com.totalIP.dto.NodeLossshow;
import com.totalIP.dto.XMLOb;

public class Lossv6XmlAction extends ActionSupport{
	...
	public String eighthourslossxml(){
		String xml = null;
		Date date = new Date();
		...
		root.setAttribute("caption", "Loss in recent 8 hours");
		...
		for(int i = 0; i <= 7; i++){
			LossDaov6 dd = new LossDaov6();
			List<NodeLossshow> nds = dd.getRecentEightHoursLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i).getLoss());
		...
		for(int i = 0; i <= 7; i++){
			...
			List<NodeLossshow> nds = dd.getRecentEightHoursLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i+1).getLoss());
		...
		for(int i = 0; i <= 7; i++){
			...
			List<NodeLossshow> nds = dd.getRecentEightHoursLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i+2).getLoss());
		...
		for(int i = 0; i <= 7; i++){
			...
			List<NodeLossshow> nds = dd.getRecentEightHoursLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i+3).getLoss());
		...
		for(int i = 0; i <= 7; i++){
			...
			List<NodeLossshow> nds = dd.getRecentEightHoursLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i+4).getLoss());
        ...
        //System.out.println(eighthoursXML);
		} catch (Exception e) {
	   ...
	   /*XMLOb xmlOb = new XMLOb();
       xmlOb.setXmlName("xmlcontent");
       totalXML.add(xmlOb);*/
		return xml;
	}
	public String onedaylossxml(){
		String xml = null;
		Date date = new Date();
        SimpleDateFormat df = new SimpleDateFormat("MM/dd/HH");
		...
		//创建根节点
		Element root = document.createElement("chart");
		root.setAttribute("caption", "Loss in recent one day");
		...
		for(int i = 0; i < 24; i++){
			LossDaov6 dd = new LossDaov6();
			List<NodeLossshow> nds = dd.getRecentOneDayLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i).getLoss());
			afrnicdatasetElement.appendChild(setElement);
		}
	...
	public String oneweeklossxml(){
		...
		//创建根节点
		Element root = document.createElement("chart");
		root.setAttribute("caption", "Loss in recent one week");
		root.setAttribute("subcaption", "(From "+onedayagodoc+" to "+doc+")");
		root.setAttribute("lineThickness", "1");
		root.setAttribute("showValues", "0");
		root.setAttribute("formatNumberScale", "0");
		...
		for(int i = 0; i < 7; i++){
			LossDaov6 dd = new LossDaov6();
			List<NodeLossshow> nds = dd.getRecentOneWeekLoss();
			Element setElement = document.createElement("set");
			setElement.setAttribute("value", ""+nds.get(5*i).getLoss());
			afrnicdatasetElement.appendChild(setElement);
		}
	...
	public String alllossxml() {
		Lossv6XmlAction d = new Lossv6XmlAction();
		XMLOb a = new XMLOb();
		a.setEighthoursxmlName(d.eighthourslossxml());
		System.out.println("11");
		a.setOnedayxmlName(d.onedaylossxml());
		System.out.println("22");
		a.setOneweekxmlName(d.oneweeklossxml());
