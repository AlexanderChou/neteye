...
public class DelayDaov6 {
	public List<NodeDelayshow> getRecentEightHoursDelay() {
		...
		String sql = "select nd.region,nd.time,avg(nd.rtt) from NodeDelay nd where nd.network_type = 6 and nd.time >= '"+eighthoursagodoc+"' group by nd.time,nd.region";
	...
	public List<NodeDelayshow> getRecentOneDayDelay() {
        ...
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		String sql = "select nd.region,nd.time,avg(nd.rtt) from NodeDelay nd where nd.network_type = 6 and nd.time >= '"+onedayagodoc+"' group by nd.time,nd.region";
		List<NodeDelayshow> DelayList = new ArrayList<NodeDelayshow>();
		List<Object[]> TempList =  session.createQuery(sql).list();
		for(int j = 0; j < TempList.size(); j++)
		{
	...
	public List<NodeDelayshow> getRecentOneWeekDelay() {
		...
		for(int i = 0; i < 7; i++)
		{
		String sql = "select nd.region,nd.time,avg(nd.rtt) from NodeDelay nd where nd.network_type = 6 and nd.time >= '"+oneweektimedoc[i+1]+"' and nd.time < '"+oneweektimedoc[i]+"' group by nd.region";
		List<Object[]> TempList =  session.createQuery(sql).list();
		for(int j = 0; j < TempList.size(); j++)
		{
			NodeDelayshow tempNode = new NodeDelayshow();
