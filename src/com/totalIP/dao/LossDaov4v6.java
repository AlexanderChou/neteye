...
public class LossDaov4v6 {
	public List<NodeLossshow> getRecentEightHoursLoss() {
		...
		String sql = "select nl.network_type,nl.time,avg(nl.loss) from NodeLoss nl where nl.time >= '"+eighthoursagodoc+"' group by nl.time,nl.network_type";
	...
	public List<NodeLossshow> getRecentOneDayLoss() {
        ...
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		String sql = "select nl.network_type,nl.time,avg(nl.loss) from NodeLoss nl where nl.time >= '"+onedayagodoc+"' group by nl.time,nl.network_type";
		List<NodeLossshow> LossList = new ArrayList<NodeLossshow>();
		List<Object[]> TempList =  session.createQuery(sql).list();
		for(int j = 0; j < TempList.size(); j++)
		{
	...
	public List<NodeLossshow> getRecentOneWeekLoss() {
		...
		for(int i = 0; i < 7; i++)
		{
		String sql = "select nl.network_type,nl.time,avg(nl.loss) from NodeLoss nl where and nl.time >= '"+oneweektimedoc[i+1]+"' and nl.time < '"+oneweektimedoc[i]+"' group by nl.network_type";
		List<Object[]> TempList =  session.createQuery(sql).list();
		for(int j = 0; j < TempList.size(); j++)
		{
			NodeLossshow tempNode = new NodeLossshow();
