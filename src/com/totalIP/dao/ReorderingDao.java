...
import com.base.model.NodeReordering;
import com.totalIP.dto.NodeReorderingshow;

public class ReorderingDao {
	public List<NodeReorderingshow> getRecentEightHoursReordering() {
		...
		String sql = "select nr.region,nr.time,avg(nr.reordering) from NodeReordering nr where nr.network_type = 4 and nr.time >= '"+eighthoursagodoc+"' group by nr.time,nr.region";
	...
	public List<NodeReorderingshow> getRecentOneDayReordering() {
        ...
        Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		Transaction transaction = session.beginTransaction();
		String sql = "select nr.region,nr.time,avg(nr.reordering) from NodeReordering nr where nr.network_type = 4 and nr.time >= '"+onedayagodoc+"' group by nr.time,nr.region";
		List<NodeReorderingshow> ReorderingList = new ArrayList<NodeReorderingshow>();
		List<Object[]> TempList =  session.createQuery(sql).list();
		session.close();
		for(int j = 0; j < TempList.size(); j++)
	...
	public List<NodeReorderingshow> getRecentOneWeekReordering() {
		...
		for(int i = 0; i < 7; i++)
		{
		String sql = "select nr.region,nr.time,avg(nr.reordering) from NodeReordering nr where nr.network_type = 4 and nr.time >= '"+oneweektimedoc[i+1]+"' and nr.time < '"+oneweektimedoc[i]+"' group by nr.region";
		List<Object[]> TempList =  session.createQuery(sql).list();
		for(int j = 0; j < TempList.size(); j++)
		{
			NodeReorderingshow tempNode = new NodeReorderingshow();
