...
Ext.onReady( function() {
	...
	var bindingTableStore = new Ext.data.Store({
		proxy : new Ext.data.HttpProxy({
			url :'show/listRealTimeInterfaceBindingTableInfo.do?ipVersion=' + ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + '&ifIndex=' + ifIndex
		}),
		reader : new Ext.data.JsonReader({
			root : 'bindingTableInfoList',
			totalProperty:'totalCount',
			fields : ['ifIndex','ipAddressType','ipAddress','bindingType','macAddress', 'bindingState', 'lifeTime','user', 'isInFilteringTable']
			})
	});
	...
	var bindingTablePanel = new Ext.grid.GridPanel({
		...
		renderTo : "showDiv",
		title : bindingTableForInterfaceText + '( ' + switchText + switchName + ', ' + indexText + ifIndex + ' )',
		border: true,
		collapsible: false,
		...
		tbar : [ '->','-','->', {
			...
			handler: function(){
				bindingTableStore.proxy = new Ext.data.HttpProxy({
					url :'show/listRealTimeInterfaceBindingTableInfoByType.do?ipVersion=' + ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + '&ifIndex=' + ifIndex + '&bindingType=1'
				});
				...
				handler: function(){
					bindingTableStore.proxy = new Ext.data.HttpProxy({
						url :'show/listRealTimeInterfaceBindingTableInfoByType.do?ipVersion=' + ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + '&ifIndex=' + ifIndex + '&bindingType=2'
					});
					bindingTableStore.reload({
						params: {
							start:0,
				...
				handler: function(){
				bindingTableStore.proxy = new Ext.data.HttpProxy({
					url :'show/listRealTimeInterfaceBindingTableInfoByType.do?ipVersion=' + ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + '&ifIndex=' + ifIndex + '&bindingType=3'
				});
				bindingTableStore.reload({
					params: {
						start:0,
