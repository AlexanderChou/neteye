...
Ext.onReady( function() {
	...
	//获取交换机的基本信息
	Ext.Ajax.request({
		url : 'show/listShowSwitchcur.do?ipVersion='+ipVersion + "&switchbasicinfoId=" + switchbasicinfoId,
		disableCaching : true,
		method : 'GET',
		success : function(result, request) {
			switchInfoForDetail=Ext.decode(result.responseText).switchInfoForDetail;
			switchName = switchInfoForDetail.switchName;
			totalUser = switchInfoForDetail.staticNum + switchInfoForDetail.slaacNum + switchInfoForDetail.dhcpNum;
			...
			var ipv4addressRec = new Ext.grid.PropertyRecord({
			    ...
			    value: switchInfoForDetail.ipv4address!=null?switchInfoForDetail.ipv4address:''
			});
			
			var ipv6addressRec = new Ext.grid.PropertyRecord({
			    name: ipv6addressText,
			    value: switchInfoForDetail.ipv6address!=null?switchInfoForDetail.ipv6address:''
			});
			
			var ipVersionRec = new Ext.grid.PropertyRecord({
			    name: ipVersionText,
			    value: switchInfoForDetail.ipVersion != null?(switchInfoForDetail.ipVersion == 1?'4':'6'):''
			});
			
			var nameRec = new Ext.grid.PropertyRecord({
			    name: nameText,
			    value: switchInfoForDetail.switchName!=null?switchInfoForDetail.switchName:''
			});
			
			var equipmentTypeRec = new Ext.grid.PropertyRecord({
			    name: equipmentTypeText,
			    value: switchInfoForDetail.equipmentType!=null?switchInfoForDetail.equipmentType:''
			});
			
			var descriptionRec = new Ext.grid.PropertyRecord({
			    name: descriptionText,
			    value: switchInfoForDetail.description!=null?switchInfoForDetail.description:''
			});
			
			var subnetNameRec = new Ext.grid.PropertyRecord({
			    name: subnetNameText,
			    value:switchInfoForDetail.subnetName!=null?switchInfoForDetail.subnetName:''
			});
			
			panel.store.add(ipv4addressRec);
			panel.store.add(ipv6addressRec);
			panel.store.add(ipVersionRec);
			panel.store.add(nameRec);
			panel.store.add(equipmentTypeRec);
			panel.store.add(subnetNameRec);
			panel.store.add(descriptionRec);
		}
	});
	...
	var userCurvesChartPanel = new Ext.Panel({
		...
		items : [{
			...
			height: 245,
			style: 'padding: 0px 10px 0px 0px',
			title : switchUserNumberChangeChartText,
			html : chartTpl.apply({pic: 'jfreechart/genSwitchUserNumberChangeChart.do?ipVersion='+ipVersion + "&switchbasicinfoId=" + switchbasicinfoId})
		}, {
			columnWidth: .5,
			height: 245,
			title : switchUserOnlineTimeChartText,
			html : chartTpl.apply({pic: 'jfreechart/genSwitchUserOnlineTimeChart.do?ipVersion='+ipVersion + "&switchbasicinfoId=" + switchbasicinfoId})
		}]
	});
	...
	var userChangePanel = new Ext.grid.GridPanel({
		...
		//列模型
		columns : [
			{header: uTimeText, width: 100, sortable: true, dataIndex: 'time'},
		...
		tools:[{
			...
			handler: function(e, target, panel){
				window.open('showSwitchUserChangeInfo.do?ipVersion='+ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + "&switchName=" + switchName);
			}
		}],
	...
	var bindingTableStore = new Ext.data.Store({
		...
		reader : new Ext.data.JsonReader({
			...
			totalProperty:'totalCount',
			fields : ['switchName','ifIndex','ipAddressType','ipAddress','macAddress','bindingType','bindingState', 'lifeTime','user', 'isInFilteringTable']
			})
	});
	...
	var bindingTableColm = new Ext.grid.ColumnModel( 
		[
			...
			{header: bIPAddressTypeText, width: 70, sortable: true, dataIndex: 'ipAddressType', sortable:false,renderer : renderIpVersion},
			...
			{header: bUserText, width:60,sortable: true,dataIndex : 'user'},
	...
	var bindingTablePanel = new Ext.grid.GridPanel({
		...
		tools:[{
			id: 'right',
			handler: function(e, target, panel){
				window.open('showRealTimeSwitchBindingTableInfo.do?ipVersion=' + ipVersion + "&switchbasicinfoId=" + switchbasicinfoId + '&switchName=' + switchName) ;
			}
		}],
		bbar:new Ext.PagingToolbar({
			pageSize:5,
